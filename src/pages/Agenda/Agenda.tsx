...
export default function Agenda(){
  ...
  useEffect(()=>{ localStorage.setItem(TIMES_KEY, JSON.stringify(extraTimes)) }, [extraTimes])
  ...
  function handleCreateService(s: Service){ setServices(prev=>[...prev, s]) }

  function handleCellClick(barberId:number, time:string){
    // abrir modal com barbeiro e horario preenchidos
    setInitialAppt({
      id: String(Math.floor(Math.random()*1000000)), barberId, clientName:'', phone:'', serviceName:'', startTime: time, duration:30, price:0, date, notes:''
    })
    setShowModal(true)
  }
  ...
  return (
    <div className="p-6">
      <Header date={dateDisplay} onNew={()=>{ setInitialAppt(null); setShowModal(true) }} />
      <div className="mb-4 flex items-center justify-between gap-4">
        <div>
          <input type="date" value={date} onChange={e=>setDate(e.target.value)} className="border p-2 rounded" />
        </div>
        <TimeSlotManager extraTimes={extraTimes} onAdd={t=>setExtraTimes(prev=>[...prev, t])} onRemove={t=>setExtraTimes(prev=>prev.filter(x=>x!==t))} />
      </div>

      <ScheduleGrid barbers={barbers} appointments={appointments} date={date} times={allTimes()} onCellClick={handleCellClick} onApptClick={handleApptClick} />

      <NewAppointmentModal visible={showModal} onClose={()=>setShowModal(false)} onSave={handleSave} onDelete={handleDelete} barbers={barbers} services={services} onCreateService={handleCreateService} date={date} initial={initialAppt} />
    </div>
  )
}
